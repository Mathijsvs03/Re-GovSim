from datetime import datetime

from simulation.persona.common import PersonaIdentity
from simulation.utils import ModelWandbWrapper
from pathfinder import assistant, system, user

from .utils import (
    consider_identity_persona_prompt,
    conversation_to_string_with_dash,
    get_sytem_prompt,
    list_to_comma_string,
    list_to_string_with_dash,
    location_time_info,
    memory_prompt,
    reasoning_steps_prompt,
)

def get_human_answer_act(prompt: str) -> str:
    print(prompt)

    while True:
        try:
            user_input = input("Please enter your answer: ")
            if user_input:
                return user_input
        except ValueError:
            print("Invalid input, please try again.")


def prompt_action_choose_amount_of_fish_to_catch(
    model: ModelWandbWrapper,
    identity: PersonaIdentity,
    memories: list[str],
    current_location: str,
    current_time: datetime,
    context: str,
    interval: list[int],
    consider_identity_persona: bool = True,
):
    lm = model.start_chain(identity.name, "fishing_cognition_act", "choose_act_options")

    with user():
        lm += f"{get_sytem_prompt(identity)}\n"
        lm += location_time_info(current_location, current_time)
        lm += memory_prompt(identity, memories)
        lm += f"\n"
        lm += f"Task: With a fishing range set between {interval[0]}-{interval[-1]}, how many tons of fish would you catch this month? "
        lm += reasoning_steps_prompt()
        lm += ' Put the final answer after "Answer:", example Answer: N tons.'

    with assistant():
        if False and identity.type == "human_agent": # Dont consider identity type for testing
            # first you go in model.gen -> previous lm + pathfinder.gen (Gen object)
            start_time_ms = datetime.now().timestamp() * 1000
            current_prompt = lm._current_prompt()
            res = get_human_answer_act(current_prompt)
            # lm += res
            # lm.set("reasoning", res)
            model.log_human_output(start_time_ms, current_prompt, lm, name="reasoning")
            option = int(res)
        else:
            lm = model.gen(
                lm,
                "reasoning",
                stop_regex=r"Answer:|So, the answer is:|\*\*Answer\*\*:",
                save_stop_text=True,
            )

            reasoning = lm["reasoning"]
            stop_patterns = ["Answer:", "So, the answer is:", "**Answer**:"]
            normalized_reasoning = reasoning.rstrip()
            last_item_matches = any(normalized_reasoning.endswith(pattern) for pattern in stop_patterns)

            if not last_item_matches:
                option = 0
            else:
                lm = model.find(
                    lm,
                    regex=r"\d+",
                    default_value="0",
                    stop_regex=f"tons",
                    name="option",
                )
                option = int(lm["option"])
                reasoning = lm["reasoning"]
            reasoning = lm["reasoning"]

    model.end_chain(identity.name, lm)

    return option, lm.html()
